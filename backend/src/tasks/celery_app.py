"""
Celery Application Configuration
Async task processing for job discovery and auto-apply
"""
from celery import Celery

from core.config import settings


# Initialize Celery app
celery_app = Celery(
    "jobapplier",
    broker=getattr(settings, 'CELERY_BROKER_URL', 'redis://localhost:6379/0'),
    backend=getattr(settings, 'CELERY_RESULT_BACKEND', 'redis://localhost:6379/0'),
    include=['tasks.job_tasks']
)

# Celery configuration
celery_app.conf.update(
    task_serializer='json',
    accept_content=['json'],
    result_serializer='json',
    timezone='UTC',
    enable_utc=True,
    task_track_started=True,
    task_time_limit=30 * 60,  # 30 minutes max
    worker_prefetch_multiplier=1,
    worker_max_tasks_per_child=1000,
)

# Task routes (optional - for multiple queues)
celery_app.conf.task_routes = {
    'tasks.job_tasks.discover_jobs_task': {'queue': 'discovery'},
    'tasks.job_tasks.apply_to_jobs_task': {'queue': 'apply'},
}
